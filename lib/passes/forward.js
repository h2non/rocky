...
module.exports = function forward(route, opts, req, res, next) {
  next = next || function () {}
  ...
  if (!opts.target) {
    return next(missingTarget(route, req, res))
  }
  ...
  function forwarder(err) {
    if (err) {
      ...
      return next(errors.replyWithError(err, res))
    }

    proxyRequest(route, opts, forwardReq, res, next)
  }
}

function proxyRequest(route, opts, forwardReq, res, next) {
