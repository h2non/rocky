...
Dispatcher.prototype.forward = function (opts, req, res) {
  ...
  route.mw.run('forward', req, res, forwarder.bind(this))
...
Dispatcher.prototype.replay = function (opts, req) {
  ...
  this.route.mw.run('replay', replayReq, replayRes, replayer.bind(this))

  function replayer(err) {
    if (err) {
      return middlewareError(err, replayReq, replayRes)
    }

    var urls = replayOpts.replays || this.route.replays
    var replays = urls && urls.length
      ? urls
      : this.rocky.replays

    replays.map(function (replay) {
      return _.assign({}, replayOpts, replay)
    })
